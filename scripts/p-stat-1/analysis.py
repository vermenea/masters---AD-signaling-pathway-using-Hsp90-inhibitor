...
from pstat1_data import pstat1_data
...
for group_data, name in zip([normalized_control, normalized_stimulation, normalized_treatment_01, normalized_treatment_1],
    ...
    print(f'Shapiro-Wilk Test for {name}: Statistic={stat:.4f}, p-value={p:.4f}')
    print(f'{name}: {"Normally distributed" if p > 0.05 else "NOT normally distributed"}')
...
print('Variances are', 'equal (ANOVA ok)' if p_levene > 0.05 else 'NOT equal (use Welch or Kruskal)')
...
ax.set_title('Phosphorylation of STAT1', fontsize=18, fontweight='bold', color='#4A235A')
ax.set_ylabel('P-STAT1 / Beta Actin', fontsize=14, color='#4A235A')
ax.set_xlabel('')
